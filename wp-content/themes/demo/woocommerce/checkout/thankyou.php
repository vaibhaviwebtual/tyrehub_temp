...
if($user_role == "Installer"){
    ...
    if($order_status == 'on-hold' || $order_status == 'processing' || $order_status == 'customprocess' || $payment_method == 'cod' || $payment_method == 'pos' || $payment_method == 'bacs' || $payment_method == 'wallet')
    {
       ...
       $customer_text = trim(preg_replace('/\s+/', ' ', $customer_text));
        ...
        $voucher_pdfname='services-voucher-'.$order_id.'.pdf';
...
<div class="woocommerce-order">
    ...
    <?php if ( $order ) : ?>
        ...
        <?php if ( $order->has_status( 'failed' ) || $order->has_status( 'pending' )) : ?>
        ...
        <?php else : ?>
            <?php
                ...
                {
                    //  echo $item_id;
                    //$vehicle_type=wc_get_order_item_meta($item_id, 'pa_vehicle-type', true );
            ...
            <?php if(($user_role == "customer" || !is_user_logged_in()) && !isset($carInfo->order_id) && $withTyreServicesPopup=='open'){?>
            <?php 
