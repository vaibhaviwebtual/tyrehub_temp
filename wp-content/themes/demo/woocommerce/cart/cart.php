			...
			<tbody>
				...
				{				
					...
					if($franchise && $cart_item['offline-purchase']!='yes'){
						$qty=10;
					}elseif($cart_item['offline-purchase']=='yes'){  // for offline customer
						if(is_array($cart_item['variation']) && $cart_item['variation']['service_data_id']== 4){
							$qty=1; // for alignment 
						}elseif($cart_item['tyre_type'] =='two-wheeler'){
							$qty=2;  // two-wheeler tyre quantity
						}
						else{
							$qty=5;
						}
					...
					if ( $_product && $_product->exists() && $cart_item['quantity'] > 0 && apply_filters( 'woocommerce_cart_item_visible', true, $cart_item, $cart_item_key ) ) {
							...
							if($current_prd == $service_voucher_prd)
							{
								...
								if(empty($franchise)){
									...
									$voucher_info = "SELECT * 
												...
												WHERE product_id = '$service_voucher_prd' AND  service_data_id=".$cart_item['services_name']." and session_id = '$session_id' AND cart_item_key='".$cart_item_key."' and order_id = ''";
						...
						?>
									<tr>
										...
										<td class="custom-voucher-remove">
											<a href="javascript:void(0);" class="delete remove-voucher" aria-label="Remove this item" data-voucher-id="<?php echo $voucher_id; ?>" data-cart-key="<?php echo $cart_item_key; ?>" ><i class="fa fa-trash"></i></a>
										</td>
									</tr>
						<?php 
								}
