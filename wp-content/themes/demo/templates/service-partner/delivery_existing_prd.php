    ...
    if(isset($_GET['total_qty']))
    {
	    ...
	    if($woocommerce->cart->cart_contents_count > 0)
	    {
	        foreach($woocommerce->cart->get_cart() as $key => $val )
	        {
	            ...
	            if($product_id == $_product->get_id() && $qty>5)
	            {		            
?>
		...
		<div class="column">
			<div class="row">
				<div class="col-xs-12">
					<h2 class="text">Get Free Home Delivery</h2>
				</div>
			</div>
					
			<div class="row">
				<div class="col-xs-12">
			    	<input type="text" placeholder="Enter pincode" name="delivery_pincode" class="delivery_pincode search_input" onfocus="this.placeholder=''" onblur="this.placeholder='Enter pincode'" />
 ...
 Congratulations you are eligible for free home delivery</span>
	...
	<div class="row descri-delivery">
		<div class="col-xs-12">
			<p>*Free delivery is available to <span>Ahmedabad</span> and <span>Gandhinagar</span> area only.<br>
				*Home delivery will take within 2-3 working days.</p>
		</div>
	</div>
</div>			
<?php 
