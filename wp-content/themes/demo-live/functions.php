...
function supplier_price_sync_to_product($products,$supplier_id=0,$product_id=0){
		...
		foreach ($products as $key => $value) {
		    			 $SQL2="SELECT count(*) as id FROM th_supplier_products_final WHERE  supplier_id='$value->supplier_id' AND product_id='$value->product_id' AND updated_date>DATE_SUB(NOW(),INTERVAL 1 YEAR)";
...
function wh_alter_pro_cat_desc() {
    if (is_product()) {
    	...
    	//print_r($product);
     ?>
        <meta name="title" content="Buy <?=$pa_brand?> <?=$pa_width?> / <?=$pa_ratio?> R<?=$pa_diameter?> <?=$vehicle_type?>  <?=$saleprice?>">
    ...
    if (is_shop()) {
    	...
    	//print_r($product);
     ?>
        <meta name="title" content="<?=$pa_brand?> Tyres | Buy <?=$pa_brand?> Tyres at the Best Price">
