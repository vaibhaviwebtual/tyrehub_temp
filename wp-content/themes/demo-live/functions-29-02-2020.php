...
function my_assets() {
	...
	if(is_cart()){
		...
		wp_enqueue_script('installer_page', get_template_directory_uri().'/assest/js/installer-page.js', array('jquery'));
		 ...
		 wp_enqueue_script('tyrehub_franchies_cartjs', get_template_directory_uri().'/assest/js/franchies_cart.js', array('jquery'));
	...
	foreach ( $cart_object->cart_contents as $key => $value )
	{
		if($value['product_id'] == $service_voucher_prd)
		{
			...
			$voucher_info = "SELECT *
					...
					WHERE product_id = '$service_voucher_prd'  AND  service_data_id=".$value['services_name']." and session_id = '$session_id' and order_id = ''";
...
function service_product_add_to_cart()
{
	...
	$cart_item_data = array('services_name'=>$_POST['service_id'],'services_price'=>$_POST['rate']);
	WC()->cart->add_to_cart($service_prd_id,1,0,'',$cart_item_data);
...
function service_voucher_product_price()
{
	...
	$barcode_string =barcode_generate();
	$SQL="DELETE from th_cart_item_service_voucher WHERE product_id = '$service_prd_id' and order_id = '' and session_id = '$session_id' and vehicle_id = '$vehicle_id' AND service_data_id=".$service_id." and installer_id = '$installer_id'";
	 ...
	 $role = $user->roles[0];
		if($role == 'Installer')
		{
			...
			if($franchise){
				...
				$myorder['offline-cartyre-purchase'] = 'Offiline Car Tyre Purchase';
				$myorder['offline-twotyre-purchase'] = 'Offiline Two Tyre Purchase';

				$myorder['franchise-cartyre-purchase'] = 'Franchise Car Tyre Purchase';
...
function installer_purchase_endpoint()
{
	...
	if($franchise){
		...
		add_rewrite_endpoint('franchise-cartyre-purchase', EP_PAGES);
...
function giftlist_page_template( $page_template )
{
	...
	if($franchise){
		...
		if(isset($wp_query->query['franchise-cartyre-purchase'])){
			$page_template = dirname( __FILE__ ).'/woocommerce/myaccount/franchise-cartyre-purchase.php' ;
		}
