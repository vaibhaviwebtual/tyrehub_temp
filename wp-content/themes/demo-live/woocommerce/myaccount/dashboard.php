...
if ( $customer_orders ) : ?>
	...
	<h2><?php echo apply_filters( 'woocommerce_my_account_my_orders_title', __($title, 'woocommerce' ) ); ?></h2>
						...
						<td class="<?php echo esc_attr( $column_id ); ?>" data-title="<?php echo esc_attr( $column_name ); ?>">
							...
							<?php elseif ( 'order-status' === $column_id ) : ?>
								...
								if(empty($franchise)){
									...
									$redirect_url = get_site_url().'/my-account/orders';
									?>
								
									<select name="order_status" class="order_status_ch" data-sel="<?php echo $order->get_order_number(); ?>" data-url="<?php echo $redirect_url; ?>">
								...
								if ( ! empty( $actions ) ) {
									foreach ( $actions as $key => $action ) {
										if(empty($franchise)){
											...
											echo '<a href="'.site_url().'/pdf-view/?document_type=invoice&&order_ids='.$order->get_order_number().'" class="btn-invert button ' . sanitize_html_class( $key ) . '">' . esc_html( $action['name'] ) . '</a>';
										}
