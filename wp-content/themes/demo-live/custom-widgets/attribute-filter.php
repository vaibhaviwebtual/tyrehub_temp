...
class custom_Layered_Nav extends WC_Widget {
	...
	public function widget( $args, $instance ) {
		...
		if($_GET['filter_vehicle-type']=='car-tyre'){
		global $wpdb;
        $SQLTWO="SELECT w_cat_id FROM th_width_display WHERE  two_wheeler='yes' AND four_wheeler==''";
        $twoRows = $wpdb->get_results($SQLTWO);
        
        foreach ($twoRows as $key => $cat) {
        	# code...
        	$excludes_ids[]=$cat->w_cat_id;
        }
        
			$get_terms_args['exclude'] = $excludes_ids;
		}

		if($_GET['filter_vehicle-type']=='two-wheeler'){
		global $wpdb;
        $SQLTWO="SELECT w_cat_id FROM th_width_display WHERE  four_wheeler='yes' AND two_wheeler ==''";
        $twoRows = $wpdb->get_results($SQLTWO);
	...
	protected function layered_nav_dropdown( $terms, $taxonomy, $query_type ) {
		...
		//echo 'shivkumar';
			//echo $taxonomy;
			//echo $this->get_current_taxonomy();
		if ( $taxonomy) {
			...
			if($taxonomy_filter_name=='width'){
			
				$current_values=str_replace("-",".",$current_values);
			}
	...
	protected function layered_nav_list( $terms, $taxonomy, $query_type ) {
		...
		echo '<ul class="woocommerce-widget-layered-nav-list shivkumar">';
		...
		$found              = false;
			$cate = get_queried_object();
 			...
 			if($_GET['filter_brand']==''){
 				/*if(!in_array($catslug, explode( ',', wc_clean( wp_unslash( $_GET['filter_brand'] ) ) ))){
		...
		foreach ( $terms as $term ) {
			...
			foreach ( $current_filter as $key => $value ) {
				//echo $this->get_current_term_slug();
