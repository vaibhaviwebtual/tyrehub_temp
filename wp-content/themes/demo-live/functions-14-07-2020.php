...
function save_service_info_from_cart()
{
	...
	if($offline=='yes'){
		...
		$update = $wpdb->get_results("UPDATE th_franchise_cart_item SET vehicle_id = '$vehicle_id' WHERE cart_item_key = '$cart_item_id'");
...
function remove_service()
{
	...
	if($franchise){
		...
		$delete_installer = $wpdb->query("DELETE from th_franchise_cart_item WHERE cart_item_key = '$cart_key'");

		$delete_service = $wpdb->query("DELETE from th_franchise_cart_item_services WHERE cart_item_key = '$cart_key'");
	}else{
...
function service_product_add_to_cart()
{
	...
	$cartd=WC()->cart->add_to_cart($service_prd_id,1,0,'',$cart_item_data);

	$session_id = WC()->session->get_customer_id();
	$data= array('cart_key'=>$cartd,'session_id'=>$session_id);
...
function action_woocommerce_checkout_order_processed( $order_id ) {
	...
	foreach ( WC()->cart->get_cart() as $cart_item_key => $cart_item )
	{
			...
			/*$SQL="SELECT * FROM th_installer_data WHERE is_franchise='yes' AND installer_data_id='".$installer_id."'";
...
function custom_dashboard_franchise() {?>
	<ul>
		...
		<li><a href="<?=admin_url()?>admin.php?page=installer-manage"> Manage Franchise</a></li>
...
function offline_product_add_to_cart()
{
	...
	$cart_item_data = array('offline-purchase'=>'yes','vehicle_type'=>$vehicle_type,'custom_data'=>array('vehicle_type'=>$vehicle_type));
	$cart_key = WC()->cart->add_to_cart($variation_id,$quantity,0,'',$cart_item_data);
