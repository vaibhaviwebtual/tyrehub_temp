...
function select_model()
{
	...
	?>
		<option value="<?php echo $model_id; ?>" <?php if(isset($_SESSION['model_id']) && $_SESSION['model_id'] == $model_id ){ echo 'selected'; }?>><?php echo $model_name; ?></option>
<?php }
	die;
}
...
?>
	...
	?>
		<option value="<?php echo $submodel_id; ?>" <?php if(isset($_SESSION['sub_model_id']) && $_SESSION['sub_model_id'] == $submodel_id ){ echo 'selected'; }?> ><?php echo $submodel_name; ?></option>
<?php }
	die;
}
...
function select_ratio()
{
	...
	?>
		<option value="<?php echo $ratio_id; ?>" <?php if($ratio_value == 0){ echo 'selected'; }?> ><?php echo $ratio_value; ?></option>
	<?php
	}
	die;
...
function my_assets() {
	...
	if(is_cart()){
		...
		wp_enqueue_script('installer_page', get_template_directory_uri().'/assest/js/installer-page.js', array('jquery'));
...
function save_installer_info()
{
	...
	$delete_service = $wpdb->get_results("DELETE from $th_cart_item_installer WHERE cart_item_key = '$cart_item_id' and session_id = '$session_id' and order_id = ''");
...
function save_service_info()
{
	...
	$delete_service = $wpdb->get_results("DELETE from $th_cart_item_services WHERE cart_item_key = '$cart_item_id' and session_id = '$session_id' and order_id = ''");
	foreach($service_list as $key => $service)
	{
		$service_arr [] = $service;
	}
	/*foreach($tyre_list as $key => $tyre)
	{
		$tyre_arr [] = $tyre;
	}*/
	for($i=0;$i<count($service_arr);$i++)
	{
		$insert = $wpdb->insert($th_cart_item_services, array(
									   ...
									   'product_id' => $product_id,
									   'vehicle_id' => $vehicle_id,
									   'service_data_id' => $service_id_list[$i],
										'service_name' => $service_arr[$i],
										'tyre' => $tyre_list,
										'rate' =>$service_rate_list[$i],
									));
	}
...
function update_tyrefitment_qty()
{
	...
	/*$SQL="UPDATE th_cart_item_services SET tyre = '$cart_item_total' WHERE session_id = '$session_id' and product_id = '$product_id' and order_id = '' and service_name = 'Tyre Fitment'";*/

	$update = $wpdb->update(
	'th_cart_item_services',
	array(
		'tyre' => $cart_item_total
	),
...
function update_servicecharge_after_update_cart()
{
	...
	$service_charge = get_woocommerce_currency_symbol().number_format($total,2,'.','');

	$content_total = $woocommerce->cart->cart_contents_total;

	$subtotal = $total+$content_total;
	 $subtotal = get_woocommerce_currency_symbol().number_format($subtotal,2,'.','');
...
function save_home_or_installer()
{
	...
	$address = $pincode;
	$url ='https://maps.googleapis.com/maps/api/geocode/json?address=' . urlencode($pincode) . '&sensor=true&key=AIzaSyCmMCGIlJKPcPn_hjlgo6j-RMfN4ZtOH70';
 ...
 // var_dump($coordinates);
   ...
   $lng = $coordinates->results[0]->geometry->location->lng;
	...
	$url = "https://maps.googleapis.com/maps/api/distancematrix/json?key=AIzaSyCmMCGIlJKPcPn_hjlgo6j-RMfN4ZtOH70&origins=".$lat.",".$lng."&destinations=23.0314594,72.56410770&mode=driving&language=pl-PL";
			$ch = curl_init();
			curl_setopt($ch, CURLOPT_URL, $url);
			...
			{
				if($dist > 20500){
					$url1 = "https://maps.googleapis.com/maps/api/distancematrix/json?key=AIzaSyCmMCGIlJKPcPn_hjlgo6j-RMfN4ZtOH70&origins=".$lat.",".$lng."&destinations=23.2283124,72.636920&mode=driving&language=pl-PL";
					$ch1 = curl_init();
...
function service_product_add_to_cart()
{
	...
	WC()->cart->add_to_cart( $service_prd_id , 1);

	echo $session_id = WC()->session->get_customer_id();
	die();
}
...
function service_voucher_product_price()
{
	...
	$barcode_string =barcode_generate();
	$SQL="DELETE from th_cart_item_service_voucher WHERE product_id = '$service_prd_id' and order_id = '' and session_id = '$session_id' and vehicle_id = '$vehicle_id' and installer_id = '$installer_id'";
...
{
	$url = "https://maps.googleapis.com/maps/api/distancematrix/json?key=AIzaSyCmMCGIlJKPcPn_hjlgo6j-RMfN4ZtOH70&origins=".$select_lat.",".$select_lng."&destinations=".$inst_lat.",".$inst_lng."&mode=driving&language=pl-PL";
	$ch = curl_init();
	curl_setopt($ch, CURLOPT_URL, $url);
	curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
	curl_setopt($ch, CURLOPT_PROXYPORT, 3128);
...
function search_by_pincode()
{
	...
	else
	{
		...
		$url = "https://maps.googleapis.com/maps/api/geocode/json?address=".urlencode($postal_code)."&sensor=false&key=AIzaSyCmMCGIlJKPcPn_hjlgo6j-RMfN4ZtOH70";
...
$qry.= ' WHERE id.visibility =1 AND (id.user_id = 55 OR id.user_id = 61) '.$where.' HAVING (distance < 15)
ORDER BY distance LIMIT 0,10';
...
ORDER BY distance LIMIT 0,10';
	...
	foreach ($row as $data)
	{
		...
		{
			$url = "https://maps.googleapis.com/maps/api/geocode/json?address=".urlencode($inst_postcode)."&sensor=false&key=AIzaSyCmMCGIlJKPcPn_hjlgo6j-RMfN4ZtOH70";
			$result_string = file_get_contents($url);
		 ...
		 $url = "https://maps.googleapis.com/maps/api/distancematrix/json?key=AIzaSyCmMCGIlJKPcPn_hjlgo6j-RMfN4ZtOH70&origins=".$select_lat.",".$select_lng."&destinations=".$inst_lat.",".$inst_lng."&mode=driving&language=pl-PL";
			...
			$install_data[$i]['facilities']=$facilities;
					...
					$SQL="SELECT meta_value from th_installer_meta where installer_id = '$installer_id' and meta_name = 'additional_services'";
					 ...
					 $as_data='';
					 $SQL="SELECT * from th_installer_addi_service ias LEFT JOIN th_service_data as sd ON sd.service_data_id=ias.service_data_id where ias.installer_id = '$installer_id' AND sd.status=1";
...
function search_by_location()
{
	...
	foreach ($row as $data)
	{
		...
		$url="https://maps.googleapis.com/maps/api/distancematrix/json?origins=".$location."&destinations=".$inst_lat.",".$inst_lng."&key=AIzaSyCmMCGIlJKPcPn_hjlgo6j-RMfN4ZtOH70";
...
function wh_phoneValidateCheckoutFields()
{
   ...
   /* $coordinates = file_get_contents('https://maps.googleapis.com/maps/api/geocode/json?address=' . urlencode($address) . '&sensor=true&key=AIzaSyCmMCGIlJKPcPn_hjlgo6j-RMfN4ZtOH70');
	...
	$url ='https://maps.googleapis.com/maps/api/geocode/json?address=' . urlencode($address) . '&sensor=true&key=AIzaSyCmMCGIlJKPcPn_hjlgo6j-RMfN4ZtOH70';
...
function login_check()
{
	...
	if (username_exists($username))
	{
		...
		if($login_with=='otp'){
			...
			$message = str_replace(' ', '%20', $message);
			$url_string="http://nimbusit.co.in/api/swsend.asp?username=t1ankitshah&password=81957032&sender=TYREHB&sendto=91".$username."&message=".$message;
			curl_setopt($ch1, CURLOPT_URL, $url_string);
			curl_setopt($ch1, CURLOPT_RETURNTRANSFER, 1);
			curl_setopt($ch1, CURLOPT_CUSTOMREQUEST, "GET");
...
function resend_otp()
{
   ...
   echo $mobile_no = $user_info->user_login;
	$ch1 = curl_init();
	$message = "We have receive your request for registration your otp is ".$otp." Thank You Tyrehub Team";
	$message = str_replace(' ', '%20', $message);
	$url_string="http://nimbusit.co.in/api/swsend.asp?username=t1ankitshah&password=81957032&sender=TYREHB&sendto=91".$mobile_no."&message=".$message;
			...
			if(!empty($row))
			{
				...
				foreach ($row as $key => $installer)
				{
					...
					if($destination == 1)
					{
						...
						$url_string="http://nimbusit.co.in/api/swsend.asp?username=t1ankitshah&password=81957032&sender=TYREHB&sendto=91".$installer_mobile."&message=".$installer_text;
			...
			if(!empty($row))
			{
				...
				// sms to customer if select installer
						...
						$url_string="http://nimbusit.co.in/api/swsend.asp?username=t1ankitshah&password=81957032&sender=TYREHB&sendto=91".$mobile_no."&message=".$customer_text;
								...
								$ch2 = curl_init();
								curl_setopt($ch2, CURLOPT_URL, $url_string);
					...
					/* -------------------- Tyrehub SMS ------------------*/
							...
							$tyrehub_text = str_replace(' ', '%20', $tyrehub_text);
								...
								curl_setopt($ch3, CURLOPT_URL, $url_string);
...
function installer_product_by_attribute()
{
	...
	{
		$message = 'No tyre profile available for your search criteria. Width: '.$width.' Ration: '.$ratio.' Diameter: '.$diameter;
		$args = array(
			'post_type' => 'product_variation',
			'posts_per_page' => -1,
			'numberposts'   => -1,
	...
	{
		$message = 'No tyre profile available for your search criteria. Width: '.$width.' Ration: '.$ratio;
	...
	{
		 $message = 'No tyre profile available for your search criteria. Width: '.$width.' Diameter: '.$diameter;
		$args = array(
			'post_type' => 'product_variation',
			'posts_per_page' => -1,
			'numberposts'   => -1,
	...
	{
		 $message = 'No tyre profile available for your search criteria. Ration: '.$ratio.' Diameter: '.$diameter;
		$args = array(
			'post_type' => 'product_variation',
			'posts_per_page' => -1,
			'numberposts'   => -1,
	...
	{
		$message = 'No tyre profile available for your search criteria. Width: '.$width;
		$args = array(
			'post_type' => 'product_variation',
			'posts_per_page' => -1,
			'numberposts'   => -1,
	...
	{
		$message = 'No tyre profile available for your search criteria. Ration: '.$ratio;
		$args = array(
			'post_type' => 'product_variation',
			'posts_per_page' => -1,
			'numberposts'   => -1,
	...
	{
		$message = 'No tyre profile available for your search criteria. Diameter: '.$diameter;
		$args = array(
			'post_type' => 'product_variation',
			'posts_per_page' => -1,
			'numberposts'   => -1,
	...
	else
	{
		...
		foreach ( $variations as $variation )
		{
			...
			if(array_key_exists($variation_ID, $prd_discount_arr))
			{
				$discount_amount = $prd_discount_arr[$variation_ID][0];
				$variation_price = $variation_price - $discount_amount;
			}
				...
				<div class="single-product demo" data-id="<?php echo $variation_ID; ?>">
					...
					<?php if($visiblity == "contact-us"){ ?>
						...
						<?php
								...
								<a href="<?php echo get_site_url(); ?>/my-account/purchase/?add-to-cart=<?php echo $variation_ID; ?>" class="btn btn-invert button product_type_simple add_to_cart_button ajax_add_to_cart" data-product_id="<?php echo $variation_ID; ?>"  data-quantity="1" aria-label="Add “ceat” to your cart" rel="nofollow"><span><i class="fa fa-shopping-cart"></i>Add to cart</span></a>
...
add_filter( 'wc_add_to_cart_message_html', 'empty_wc_add_to_cart_message');
	...
	function custom_redirect_function()
	{
		...
		if($role == 'Installer')
		{
			...
			return get_permalink( $myaccount_page_id ).'/purchase';
		}
		else{
...
function lost_pass_send_otp()
{
	...
	if($user_id)
	{
		...
		$url_string="http://nimbusit.co.in/api/swsend.asp?username=t1ankitshah&password=81957032&sender=TYREHB&sendto=91".$mobile_no."&message=".$reset_pass_msg;
		curl_setopt($ch1, CURLOPT_URL, $url_string);
		curl_setopt($ch1, CURLOPT_RETURNTRANSFER, 1);
		curl_setopt($ch1, CURLOPT_CUSTOMREQUEST, "GET");
...
//unset($_SESSION['pic_address']);
	...
	$url ='https://maps.googleapis.com/maps/api/geocode/json?address=' . urlencode($pic_address) . '&sensor=true&key=AIzaSyCmMCGIlJKPcPn_hjlgo6j-RMfN4ZtOH70';
	$ch = curl_init();
	curl_setopt( $ch, CURLOPT_URL, $url );
	curl_setopt( $ch, CURLOPT_RETURNTRANSFER, true );
	$response = curl_exec( $ch );
   // var_dump($response);
	...
	$url = "https://maps.googleapis.com/maps/api/distancematrix/json?key=AIzaSyCmMCGIlJKPcPn_hjlgo6j-RMfN4ZtOH70&origins=".$inst_lat.",".$inst_lng."&destinations=".$pic_lat.",".$pic_lng."&mode=driving&language=pl-PL";
			$ch = curl_init();
			curl_setopt($ch, CURLOPT_URL, $url);
			curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
			curl_setopt($ch, CURLOPT_PROXYPORT, 3128);
...
function supplier_price_sync_to_product($products,$supplier_id=0,$product_id=0){
		...
		foreach ($products as $key => $value) {
		    			 $SQL2="SELECT * FROM th_supplier_products_final WHERE  supplier_id='$value->supplier_id' AND product_id='$value->product_id' AND updated_date>DATE_SUB(NOW(),INTERVAL 1 YEAR)";
					...
					if($supplier_id>0 && $product_id>0){
							$SQL1="SELECT * FROM `th_supplier_products_final` where product_id='$product_id' AND supplier_id='$supplier_id' AND updated_date>=DATE_SUB(NOW(),INTERVAL 1 YEAR)";
	    			}else{

	    				$SQL1="SELECT * FROM `th_supplier_products_final` where product_id='$product_id' AND updated_date>DATE_SUB(NOW(),INTERVAL 1 YEAR)  GROUP by id HAVING min(tyre_price+tube_price) ORDER BY (tyre_price+tube_price) ASC LIMIT 0,1";
	    			}
...
function customer_share_link()
{
	...
	$url_string="http://nimbusit.co.in/api/swsend.asp?username=t1ankitshah&password=81957032&sender=TYREHB&sendto=91".$share_mobile."&message=".$message;
	curl_setopt($ch1, CURLOPT_URL, $url_string);
	curl_setopt($ch1, CURLOPT_RETURNTRANSFER, 1);
	curl_setopt($ch1, CURLOPT_CUSTOMREQUEST, "GET");
	$result1 = curl_exec($ch1);
...
function free_tyre_inspection()
{
	...
	$wpdb->insert('th_free_tyre_inspection',array (
		...
		$message = str_replace(' ', '%20', $message);
		$url_string="http://nimbusit.co.in/api/swsend.asp?username=t1ankitshah&password=81957032&sender=TYREHB&sendto=917575041221&message=".$message;
		curl_setopt($ch1, CURLOPT_URL, $url_string);
		curl_setopt($ch1, CURLOPT_RETURNTRANSFER, 1);
		curl_setopt($ch1, CURLOPT_CUSTOMREQUEST, "GET");
