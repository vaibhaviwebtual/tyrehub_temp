...
class suppliers_product_choose_List_Table extends WP_List_Table {
	...
	function __construct(){
	  ...
	  $SQL="SELECT   sp.* FROM th_supplier_products_list AS sp ";
    ...
    $WHERE.=" AND ( mt4.meta_key = 'tyrehub_visible' AND mt4.meta_value IN ('yes','contact-us')) ";
     	...
     	if($supplier_id){
		 $WHERE.=" AND sp.product_id NOT IN (SELECT product_id FROM th_supplier_products_list WHERE supplier_id =".$supplier_id.")";
		}

    $WHERE.=" AND sp.visiblity=1 AND wp_posts.post_type = 'product_variation' AND (wp_posts.post_status = 'publish' OR wp_posts.post_status = 'wc-deltoinstaller' OR wp_posts.post_status = 'wc-customprocess' OR wp_posts.post_status = 'future' OR wp_posts.post_status = 'draft' OR wp_posts.post_status = 'pending' OR wp_posts.post_status = 'private') GROUP BY wp_posts.ID ORDER BY sp.status desc";
    ...
    $supplierProductData=$wpdb->get_results($SQL);
			...
			foreach ($supplierProductData as $key => $value)
			{
				...
				$supp_id=get_post_meta($value->product_id,'active_supplier',true);
				$SQL="SELECT business_name FROM th_supplier_data WHERE supplier_data_id='".$supp_id."'";
				...
				//$suppliers_products[$key]['mrp_price']= $value->old_mrp;
				//$suppliers_products[$key]['total_price']= $value->old_total_price;
				/*if($supplier_id==$value->supplier_id){
				$suppliers_products[$key]['status']= '<a href="'.get_admin_url().'/admin.php?page=supplier-per-change&action=edit&supp_data_id='.$value->id.'" class="button-secondary woocommerce-save-button add-installer-btn">Assigned</a>';	
				}else{
				$suppliers_products[$key]['status']= '<a href="'.get_admin_url().'/admin.php?page=supplier-product-add&action=prod_add&supplier_id='.$supplier_id.'&supp_data_id='.$value->id.'" class="button-primary woocommerce-save-button add-installer-btn">Assign</a> ';	
				}*/
			$suppliers_products[$key]['status']= '<a href="'.get_admin_url().'/admin.php?page=supplier-product-add&action=prod_add&supplier_id='.$supplier_id.'&supp_data_id='.$value->id.'" class="button-primary woocommerce-save-button add-installer-btn">Assign</a> ';
		...
		function supplier_product_assigned_by_admin($id,$supplier_id){

			global $wpdb, $woocommerce;

			$SQL="SELECT * FROM th_supplier_products_list WHERE id='".$id."'";
			$result=$wpdb->get_row($SQL);

	$SQL1="SELECT * FROM th_supplier_products_list WHERE product_id='".$result->product_id."' AND supplier_id='".$supplier_id."'";
			$result1=$wpdb->get_row($SQL1);

 			

 			$supp_pro_id=$id;
 			if(empty($result1->product_id)){
 				
 				$wpdb->insert('th_supplier_products_list', array(
				    ...
				    'new_tyre_price' => 0,
				    'old_tyre_price' => 0,
				    'flat_percentage' => $result->flat_percentage,
				    'margin_price' => $result->margin_price,
				    'new_mrp' =>0,
				    'old_mrp' =>0,
				    'new_total_price' =>0,
				    'old_total_price' =>0,
			...
			/*$wpdb->update('th_supplier_products', 
				array( 
					'user_id' =>get_current_user_id(),
					'supplier_id' =>$supplier_id,
				), 
	...
	function get_columns(){
	        $columns = array(
	            ...
	            'supplier' => __( 'Active Supplier', 'mylisttable' ),
	...
	function process_bulk_action() {
		...
		if ('assign' === $this->current_action() ) {			
			...
			foreach ( $delete_ids as $id ) {
				self::supplier_product_assigned_by_admin($id,$supplier_id);
			}
		}

		if ('prod_add' === $this->current_action() ) {			
			// In our file that handles the request, verify the nonce.
			$nonce = esc_attr( $_REQUEST['_wpnonce'] );
...
function supplier_products_list(){
  
  $suppliersProductListTable = new suppliers_product_choose_List_Table();
  $supplier_id=$_REQUEST['supplier_id'];
  $res=get_supplier_data($supplier_id);
  ?>  
  <div class="wrap"><h2>Suppliers Product Not Assign List 
 ...
 <div style="margin-top:10px;"><b>Supplier : <?=$res[0]->business_name;?></b> </div>
  ...
  <form method="post">
    ...
    <?php $suppliersProductListTable->prepare_items();?>
    <?php
    //$suppliersProductListTable->search_box( 'search', 'search_id' );
    $suppliersProductListTable->display(); 
    echo '</form></div>';
}

function supplier_product_assign(){
