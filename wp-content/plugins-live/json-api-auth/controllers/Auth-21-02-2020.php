...
public function __construct() {
	...
	public function lostPassSendOtp() {
		...
		if ( !empty( $user ) ) {
			...
			$url_string = "http://nimbusit.co.in/api/swsend.asp?username=t1ankitshah&password=81957032&sender=TYREHB&sendto=91" . $json_api->query->mobile . "&message=" . $reset_pass_msg;
					...
					if ( !empty( $row ) ) {
						...
						foreach ( $row as $key => $data ) {
							...
							$vehicle_id = $data->vehicle_id;
								...
								$SQL= "SELECT * FROM th_vehicle_type WHERE vehicle_id='".$vehicle_id."'";
								$vehicle = $wpdb->get_row($SQL);
								$voucher_name='';
								$voucher_name = (!empty($vehicle->vehicle_type)) ? $vehicle->vehicle_type : '';
							if ( $order_prd_id == $service_voucher_prd ) {
								$service_voucher_id = $data->service_voucher_id;
								...
								if ( $voucher_name == 'promotional' || $voucher_name == 'promotion' ) {
									...
									$service_type = $voucher_name; //Service voucher for vehicle type
									$voucher_type = 'service';
								}
		...
		if($flag!=0){
		           ...
		           $voucher_name = $barcode->voucher_name;
                    ...
                    FROM th_service_data WHERE as_flag =0 and status =1 AND show_on_cart=1";
									...
									//Selected Services
			$services = "SELECT sd.service_name as sd_name,sd.services_image,cis.* 
                    FROM th_cart_item_service_voucher cis LEFT JOIN th_service_data as sd ON sd.service_data_id=cis.service_data_id
                    WHERE cis.product_id = '$order_prd_id' and cis.order_id = '$order_id'";
