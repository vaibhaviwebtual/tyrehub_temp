...
function wcpdf_get_document( $document_type, $order, $init = false,$generateInvo=false) {
	...
	if ( !empty( $order ) ) {
		...
		// orderless document (used as wrapper for bulk, for exampsle)
		$document = WPO_WCPDF()->documents->get_document( $document_type, $order );
	}
...
function wcpdf_get_credit_notes( $order, $init = false ) {
	return wcpdf_get_document( 'credit-notes', $order, $init );
}
