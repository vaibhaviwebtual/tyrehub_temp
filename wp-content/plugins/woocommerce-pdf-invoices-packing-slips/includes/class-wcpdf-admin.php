...
class Admin {
	...
	public function add_listing_actions( $order ) {
		...
		$documents = WPO_WCPDF()->documents->get_documents();
			...
			foreach ($documents as $document) {
				...
				}elseif(in_array($order->get_status(),array('cancelled','failed','refunded')) && $document->get_type()=='credit-notes') {
					$document->read_data( $order );
					$listing_actions['credit-notes'] = array(
						'url'		=> wp_nonce_url( admin_url( "admin-ajax.php?action=generate_wpo_wcpdf&document_type=credit-notes&order_ids=" . WCX_Order::get_id($order)), 'generate_wpo_wcpdf' ),
						'img'		=> !empty($document->icon) ? $document->icon : WPO_WCPDF()->plugin_url() . "/assets/images/generic_document.png",
						'alt'		=> "PDF " . $document->get_title(),
						'exists'	=> $document->exists(),
					);
	...
	public function add_meta_boxes() {
		...
		/*add_meta_box(
			'wpo_wcpdf-credit-data-input-box',
			__( 'PDF Credit Note data', 'woocommerce-pdf-credit-notes-packing-slips' ),
			array( $this, 'credit_data_input_box_content' ),
			'shop_order',
			'normal',
			'default'
		);*/
	...
	/**
	 * Add metabox for credit number & date
	 */
	public function credit_data_input_box_content ( $post ) {
		$order = WCX::get_order( $post->ID );

		do_action( 'wpo_wcpdf_credit_meta_box_start', $post->ID );
		
		if ( $credit_notes = wcpdf_get_credit_notes( $order ) ) {
			
			$credit_notes_number = $credit_notes->get_number();
			$credit_date = $credit_notes->get_date();
			?>
			<div class="wcpdf-data-fields" data-document="invoice" data-order_id="<?php echo WCX_Order::get_id( $order ); ?>">
				<h4><?php echo $credit_notes->get_title(); ?><?php if ($credit_notes->exists()) : ?><span class="wpo-wcpdf-edit-date-number dashicons dashicons-edit"></span><span class="wpo-wcpdf-delete-document dashicons dashicons-trash" data-nonce="<?php echo wp_create_nonce( "wpo_wcpdf_delete_document" ); ?>"></span><?php endif; ?></h4>

				<!-- Read only -->
				<div class="read-only">
					<?php if ($credit_notes->exists()) : ?>
					<div class="invoice-number">
						<p class="form-field _wcpdf_invoice_number_field ">	
							<p>
								<span><strong><?php _e( 'Credit Note Number', 'woocommerce-pdf-credit-notes-packing-slips' ); ?>:</strong></span>
								<span><?php if (!empty($credit_notes)) echo $credit_notes->get_formatted(); ?></span>
							</p>
						</p>
					</div>
					...
					<div class="invoice-date">
						<p class="form-field form-field-wide">
							<p>
								<span><strong><?php _e( 'Credit Note Date:', 'woocommerce-pdf-credit-notes-packing-slips' ); ?></strong></span>
								<span><?php if (!empty($credit_date)) echo $credit_date->date_i18n( wc_date_format().' @ '.wc_time_format() ); ?></span>
							</p>
						</p>
					</div>
					<?php else : ?>
					<span class="wpo-wcpdf-set-date-number button"><?php _e( 'Set credit notes number & date', 'woocommerce-pdf-credit-notes-packing-slips' ) ?></span>
					<?php endif; ?>
				</div>
				...
				<div class="editable">
					<p class="form-field _wcpdf_invoice_number_field ">
						<label for="_wcpdf_invoice_number"><?php _e( 'Credit Note Number (unformatted!)', 'woocommerce-pdf-invoices-packing-slips' ); ?>:</label>
						<?php if ( $credit_notes->exists() && !empty($credit_notes) ) : ?>
						<input type="text" class="short" style="" name="_wcpdf_credit_notes_number" id="_wcpdf_credit_notes_number" value="<?php echo $invoice_number->get_plain(); ?>" disabled="disabled">
						<?php else : ?>
						<input type="text" class="short" style="" name="_wcpdf_credit_notes_number" id="_wcpdf_credit_notes_number" value="" disabled="disabled">
						<?php endif; ?>
					</p>
					<p class="form-field form-field-wide">
						<label for="wcpdf_invoice_date"><?php _e( 'Credit Note Date:', 'woocommerce-pdf-invoices-packing-slips' ); ?></label>
						<?php if ( $credit_notes->exists() && !empty($credit_date) ) : ?>
						<input type="text" class="date-picker-field" name="wcpdf_credit_notes_date" id="wcpdf_credit_notes_date" maxlength="10" value="<?php echo $credit_date->date_i18n( 'Y-m-d' ); ?>" pattern="[0-9]{4}-(0[1-9]|1[012])-(0[1-9]|1[0-9]|2[0-9]|3[01])" disabled="disabled"/>@<input type="number" class="hour" placeholder="<?php _e( 'h', 'woocommerce' ) ?>" name="wcpdf_credit_notes_date_hour" id="wcpdf_invoice_date_hour" min="0" max="23" size="2" value="<?php echo $credit_date->date_i18n( 'H' ) ?>" pattern="([01]?[0-9]{1}|2[0-3]{1})" />:<input type="number" class="minute" placeholder="<?php _e( 'm', 'woocommerce' ) ?>" name="wcpdf_credit_notes_date_minute" id="wcpdf_credit_notes_date_minute" min="0" max="59" size="2" value="<?php echo $credit_date->date_i18n( 'i' ); ?>" pattern="[0-5]{1}[0-9]{1}" />
						<?php else : ?>
						<input type="text" class="date-picker-field" name="wcpdf_credit_notes_date" id="wcpdf_credit_notes_date" maxlength="10" disabled="disabled" value="" pattern="[0-9]{4}-(0[1-9]|1[012])-(0[1-9]|1[0-9]|2[0-9]|3[01])" />@<input type="number" class="hour" disabled="disabled" placeholder="<?php _e( 'h', 'woocommerce' ) ?>" name="wcpdf_credit_notes_date_hour" id="wcpdf_credit_notes_date_hour" min="0" max="23" size="2" value="" pattern="([01]?[0-9]{1}|2[0-3]{1})" />:<input type="number" class="minute" placeholder="<?php _e( 'm', 'woocommerce' ) ?>" name="wcpdf_credit_notes_date_minute" id="wcpdf_credit_notes_date_minute" min="0" max="59" size="2" value="" pattern="[0-5]{1}[0-9]{1}" disabled="disabled" />
						<?php endif; ?>
					</p>
				</div>
			</div>
		...
		do_action( 'wpo_wcpdf_credit_meta_box_end', $post->ID );
	}
