	...
	public function attach_pdf_to_email ( $attachments, $email_id, $order ) {
		...
		foreach ( $attach_to_document_types as $document_type ) {
			...
			try {
				...
				//wpo_wcpdf_send_s3bucket( $pdf_path, $filename);
				//do_action( 'wpo_wcpdf_send_s3bucket', $pdf_path, $filename);
				// get pdf voucher data & store 
	...
	public function generate_pdf_ajax() {
		...
		try {
			...
			if ( $document ) {
				...
				$filename = $document->get_filename();
				$pdf_path = $tmp_path . $filename;
				WPO_WCPDF()->wpo_wcpdf_send_s3bucket($pdf_path,$document->get_filename());
				die;*/
			} else {
