	...
	*/
	function saveTxnResponse($data  = array(),$order_id, $id = false){
		...
		$transaction_id 	= (!empty($data['TXNID'])? $data['TXNID']:'');
               ...
               if($data['PAYMENTMODE']=='DC'){
                    $pay_type='DC';
               }elseif($data['PAYMENTMODE']=='PPI'){
                    $pay_type='PPI';
               }elseif($data['PAYMENTMODE']=='NB'){
                    $pay_type='NB';
               }elseif($data['PAYMENTMODE']=='CC'){
                    $pay_type='CC';
               }else{
                    $pay_type='NA';
               }
			...
			if($status == 1){
				global $wpdb;
		        $SQLProcedure="CALL  FranchiseProfitChange(".$order_id.",'".$pay_type."')";       
