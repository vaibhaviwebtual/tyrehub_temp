...
class Installers_change_price_log_Table extends WP_List_Table {
	function get_change_price_log(){
   	$s=$_POST['s'];
	global $woocommerce , $wpdb;
	    ...
	    $sql = "SELECT DISTINCT ppl.*,sd.business_name FROM th_supplier_products_log as ppl";
		...
		{
			$price_log[$key]['log_id'] = $value->log_id;
	        ...
	        $old_tube_price = $price_log[$key]['old_tube_price']= $value->old_tube_price;
	        if($tyre_type == 'tubetyre')
        	{
				//wc_price( $log_data->old_price, $price_args );
				if($value->new_tube_price>0){
				$price_log[$key]['new_tube_price']= 'New :'.wc_price( $value->new_tube_price, $price_args ).'<br>Old :'.wc_price( $old_tube_price, $price_args );
				$tube_price	=$value->new_tube_price;
				}else{				
				$price_log[$key]['new_tube_price']=wc_price(get_post_meta($product_id , 'tube_price' , true), $price_args );
				$tube_price	=$old_tube_price;
				}
			
			}
			else{
				$price_log[$key]['new_tube_price']= '-';
			}

			$old_tyre_price = $price_log[$key]['old_tyre_price']= $value->old_tyre_price;
			if($value->new_tyre_price==0){
			$price_log[$key]['new_tyre_price']=wc_price( $old_tyre_price, $price_args );
			$tyre_price=$old_tyre_price;	
			}else{
				$price_log[$key]['new_tyre_price']= 'New :'.wc_price($value->new_tyre_price, $price_args ).'<br>Old :'.wc_price( $old_tyre_price, $price_args );
				$tyre_price=$value->new_tyre_price;
			}

			$old_percentage = $price_log[$key]['old_percentage']= $value->old_percentage;
			if($value->flat_percentage==0){
			$price_log[$key]['percentage']=$old_percentage;	
			$percentage=$old_percentage;
			}else{
				$price_log[$key]['percentage']= 'New : '.$value->flat_percentage.'<br>Old : '.$old_percentage;
				$percentage=$value->flat_percentage;
			}


			$old_margin_price =  $value->old_margin_price;
			if($value->margin_price==0){
			$price_log[$key]['margin']=wc_price($old_margin_price, $price_args );
			$margin_price=$old_margin_price;	
			}else{
				$price_log[$key]['margin']= 'New :'.wc_price($value->margin_price, $price_args ).'<br>Old :'.wc_price( $old_margin_price, $price_args );
				$margin_price=$value->margin_price;
			}
			


			$old_sale_price = $price_log[$key]['old_sale_price']= $value->old_total_price;
			if($value->new_total_price==0){

				$saleprice=wc_price($tube_price+$tyre_price+(($tyre_price*$percentage)/100)+$margin_price);
				$price_log[$key]['new_sale_price']=$saleprice;
			}else{
			$price_log[$key]['new_sale_price']= 'New :'.wc_price($value->new_total_price, $price_args ).'<br>Old :'.wc_price( $old_sale_price, $price_args );
		   }


			$old_mrp_price = $price_log[$key]['old_mrp_price']= $value->old_mrp;
			if($value->new_mrp == 0){
				//$price_log[$key]['new_mrp_price']= wc_price($old_mrp_price, $price_args );

				$price_log[$key]['new_mrp_price']=wc_price(get_post_meta($product_id , '_regular_price' , true), $price_args );
			}
			else{
				$price_log[$key]['new_mrp_price']= 'New :'.wc_price($value->new_mrp, $price_args ).'<br>Old :'.wc_price( $old_mrp_price, $price_args );
			}
			...
			$price_log[$key]['user_id']= $approve_by.'('.date('d-m-Y',strtotime($value->updated_date)).') '. $status;	
    ...
    function get_columns(){
        $columns = array(
            ...
            'new_tube_price' => __( 'Tube Price', 'mylisttable' ),
            'new_tyre_price' => __( 'Tyre Price', 'mylisttable' ),
            'percentage' => __( 'Per(%)', 'mylisttable' ),
            'margin' => __( 'Margin', 'mylisttable' ),
            'new_sale_price' => __( 'Sale Price', 'mylisttable' ),
            'new_mrp_price' => __( 'M.R.P. Price', 'mylisttable' ),
    ...
    function prepare_items() {
	  ...
	  $data=$this->get_change_price_log();
...
function change_price_log(){
	...
	$priceLogTable = new Installers_change_price_log_Table();
	global $woocommerce , $wpdb;
    $log_sql = $wpdb->get_results("SELECT DISTINCT product_id from th_change_product_price_log order by log_id desc");
...
function search_prd_for_log()
{
    ...
    $log_sql = $wpdb->get_results("SELECT * from th_change_product_price_log WHERE `date`>= DATE_FORMAT(CURRENT_DATE - INTERVAL $months MONTH, '%Y-%m-%d' ) order by log_id DESC");	
	}
...
add_action('wp_ajax_product_name_dropdown', 'product_name_dropdown');
add_action('wp_ajax_nopriv_product_name_dropdown', 'product_name_dropdown');
function product_name_dropdown()
{
	...
	global $woocommerce , $wpdb;
	   ...
	   $months = $_POST['months'];
		...
		$sql = "SELECT DISTINCT ppl.product_id FROM th_supplier_products_log as ppl";
