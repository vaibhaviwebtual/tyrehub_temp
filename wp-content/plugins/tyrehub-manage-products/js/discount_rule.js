jQuery(document).ready(function($) 
{
    ...
    $(document).on('click','#supplier_prodct_search',function(e){
        e.preventDefault();
        var width = $('.width').val();
        var ratio = $('.ratio').val();
        ...
        $.ajax({    
                ...
                beforeSend: function() {                        
                        $('.product-details-list').append(loader);
                    },
                success: function (data) {
                   
                    $('.product-details-list').html(data);
                },
            });
    });
     ...
     $(document).on('keyup','.rule-name',function(){
        ...
        {
            $('.save_discount').removeAttr('disabled');
        }
        else{
            $('save_discount').attr('disabled','disabled');
        }
    });
                            ...
                            if(discount_amount != '')
                            {
                                ...
                                $.ajax({    
                                    ...
                                    success: function (data) {
                                      ...
                                      // location.reload();
                                       window.location.replace('/my-account/deals-discount/');
                                    },
                                });
                            }
                        });
    ...
    $(document).on('click','.discount-rule .selected-products .single-product .remove',function()
    {
        if(confirm('Are you sure you want to remove discount?'))
        {
            ...
            var tubeprice = $(this).parent().find('.tube-price').html();
            var tyreprice = $(this).parent().find('.tyre-price').html();
            //var price = $(this).parent().find('.sale-price').html();
            var price =$(this).parent().find('.sale-price').html();
            ...
            if($.type(search_res) === "undefined")
            {
                ...
                temp_html +='<div class="price">'+tubeprice+'</div>';
                temp_html +='<div class="price">'+tyreprice+'</div>';
                temp_html +='<div class="price">'+price+'</div>';
                temp_html +='<div class="amount">';
                temp_html +='<input type="text" name="discount_amount" class="discount_amount" value="'+amount+'">';
                temp_html +='</div><div class="send"><span>>></span></div></div>';  

                $( ".product-details-list" ).append(temp_html);
            }
            else{
                    ...
                    $(".product-container .selected-products .single-product").each(function()
                    {
                        ...
                        {                           
                            ...
                            $.ajax({    
                                ...
                                success: function (data) {
                                   // $('.product-details').html(data);
                                   $('.message-block').html('<div class="notice notice-success"><p>Discount rule updated!</p></div>');
                                   $('html, body').animate({
...
$(document).on('change','.select-update-by',function(e){
   ...
   $(document).on('keyup','.bulk-tube-price',function(){
        var added_price = $(this).val();
        var update_by = $(".select-update-by option:selected").val();
        $(".selected-products .single-product").each(function()
        {
           // console.log($(this).attr('data-id'));
            var sale_price = $(this).find('.sale-price').attr('data-sale-price');

            if(update_by == 'productvalue'){

                var new_discount_price = parseInt(added_price);
            }
            else{
                var per = added_price;
                var new_discount_price = parseInt(sale_price)*parseInt(per)  / 100;
               
            }

           

            $(this).find('.discount_amount').val(new_discount_price.toFixed(0));
        });
    });
