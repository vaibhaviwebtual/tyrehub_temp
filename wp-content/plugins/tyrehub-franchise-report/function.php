...
function admin_franchise_offline_orders()
{
    ...
    $currency = get_woocommerce_currency_symbol();   
        ...
        $SQL="SELECT * FROM wp_franchises_order as fo 
		INNER JOIN wp_franchise_order_items as foi ON fo.order_id = foi.order_id 
		where 1=1  $where_search AND fo.is_deleted='0' ORDER BY fo.order_id DESC LIMIT 0,50";
        //echo $SQL;
        ...
        // echo $long = get_post_meta("3375","attribute_pa_brand",true);
			$i=0;
           foreach( $all_posts as $key => $post ):            
			...
			//$align = get_alignment_offline_data($post->order_number);
			//$balancing = get_alignment_offline_data($post->order_number);
			//$carwash = get_alignment_offline_data($post->order_number);
			//echo count($alignment);
			/*$align = "";
			$balancing ="";
			$carwash ="";
			for($i=1;$i<=count($alignment);$i++){
				//echo "<pre>";
				//print_r($alignment[$i]->service_data_id);
				if ($alignment[$i]->service_data_id == 2)
				{$balancing = "1";}
				else {$balancing = "0";}
				if ($alignment[$i]->service_data_id == 3)
				{$align = "1";}
				else {$align = "0";}
				if ($alignment[$i]->service_data_id == 5)
				{$carwash = "1";}
				else {$carwash = "0";}
			}*/
            ...
            $status=($post->status==1)? 'Pending' : 'Completed';
                ...
                <tr class="order" id="odr'.$post->order_id.'">
					...
					<td>'.$size.'</td>
					<td>'.$qtys.'</td>
					<td>'.get_alignment_offline_data($post->order_number,3).'</td>
					<td>'.get_alignment_offline_data($post->order_number,2).'</td>
					<td>'.get_alignment_offline_data($post->order_number,5).'</td>
					<td>'.$vtype.'</td>
                    <td>'.$status.'</td>
...
function get_alignment_offline_data($order_id,$j){
  global $wpdb;
  $align ="SELECT * FROM th_franchise_cart_item_services WHERE order_id = $order_id and service_data_id= $j";
  $alignment_balancing = $wpdb->get_row($align);
...
function get_franchise_meta( $post_id, $meta_key ) {	
  global $wpdb;
  $mid ="SELECT * FROM wp_francise_order_itemmeta WHERE order_item_id = $post_id AND meta_key = '$meta_key'";
  $franchise=$wpdb->get_row($mid);
 // echo "<pre>";
  //print_r($franchise);
 // print_r($franchise->meta_value);
  if( $franchise != '' ){
    //return $mid;
	return $franchise->meta_value;
  }
  return false;
}
