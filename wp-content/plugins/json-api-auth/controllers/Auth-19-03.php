...
class JSON_API_Auth_Controller
{
    ...
    public function validate_auth_cookie()
    {
        ...
        $valid = wp_validate_auth_cookie($json_api
            ->query->cookie, 'logged_in') ? true : false;
        return array(
            "valid" => $valid
        );
    }
	/* Add code not in default plugin */
	    ...
	    {	
	        ...
	        $user = wp_authenticate($json_api	
	            ->query->username, $json_api	
	            ->query	
	            ->password);	
	        ...
	        {	
	            ...
	            remove_action('wp_login_failed', $json_api	
	                ->query	
	                ->username);	
	    ...
	    {	
	        ...
	        $user = get_userdatabylogin($json_api	
	            ->query	
	            ->mobile);	
	        ...
	        {	
	            ...
	            $delSQL = "DELETE from th_lost_password WHERE mobile_no = '" . $json_api	
	                ->query->mobile . "'";	
	            ...
	            $url_string = "http://nimbusit.co.in/api/swsend.asp?username=t1ankitshah&password=81957032&sender=TYREHB&sendto=91" . $json_api	
	                ->query->mobile . "&message=" . $reset_pass_msg;	
	            ...
	            $respond['data'] = array(	
	                'id' => $user->ID,	
	                'installer_id' => $installer->installer_data_id,	
	                'otp' => $otp	
	            );	
	    ...
	    {	
	        ...
	        wp_set_password($json_api	
	            ->query->newpass, $json_api	
	            ->query	
	            ->id);	
	    ...
	    {	
	        ...
	        $user = get_user_by('id', $json_api	
	            ->query	
	            ->login_id);	
		
	        if ($user && wp_check_password($json_api	
	            ->query->oldpass, $user	
	    ...
	    {	
	        ...
	        $center_name = $json_api	
	            ->query->center_name;	
		
	        $store_phone = $json_api	
	            ->query->store_phone;	
		
	        $contact_person_name = $json_api	
	        ...
	        $company_name = $json_api	
	            ->query->company_name;	
		
	        $company_add = $json_api	
	            ->query->company_add;	
	        ...
	        {	
	            ...
	            $where = array(	
	                'installer_data_id' => $installer_id	
    ...
    public function get_currentuserinfo()
    {
        ...
        $user_id = wp_validate_auth_cookie($json_api
            ->query->cookie, 'logged_in');
        if (!$user_id)
        {
	...
	/* second code add by us not in default plugin */
