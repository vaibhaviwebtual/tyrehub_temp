...
abstract class Order_Document_Methods extends Order_Document {
	...
	public function get_credit_notes_number() {
		// Call the woocommerce_invoice_number filter and let third-party plugins set a number.
		// Default is null, so we can detect whether a plugin has set the invoice number
		$third_party_credit_notes_number = apply_filters( 'woocommerce_credit_notes_number', null, $this->order_id );
		if ($third_party_credit_notes_number !== null) {
			return $third_party_credit_notes_number;
		}

		if ( $credit_notes_number = $this->get_number('credit-notes') ) {
			
			return $formatted_credit_notes_number = $credit_notes_number->get_formatted();
		} else {
			return '';
		}
	}
	...
	public function credit_notes_number() {
		echo $this->get_credit_notes_number();
	}
