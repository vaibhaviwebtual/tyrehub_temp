...
class WC_Gateway_BACS extends WC_Payment_Gateway {
	...
	private function bank_details( $order_id = '' ) {
		...
		// code by bv
	     ...
	     $store_code = $franchise->user_code;
				...
				foreach ( $account_fields as $field_key => $field ) {
					...
					if($franchise){
						if($field_key == 'iban')
						{
							...
							$account_html .= '<li class="' . esc_attr( $field_key ) . '">' . wp_kses_post('Remarks') . ': <strong>' . wp_kses_post( wptexturize('Pay from '.$store_code) ) . '</strong></li>' . PHP_EOL;
