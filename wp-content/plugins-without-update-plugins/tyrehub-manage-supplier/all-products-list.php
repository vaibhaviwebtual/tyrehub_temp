...
class Products_List_Table extends WP_List_Table {
	...
	if($vehicle_type){
	 ...
	 if($supplier_id){
	 	$SQL.=" LEFT JOIN wp_postmeta AS mt6 ON ( wp_posts.ID = mt6.post_id )";
	 }
...
*/
	if($supplier_id){
    	$WHERE.=" AND ( mt6.meta_key = 'active_supplier' AND mt6.meta_value IN ('".$supplier_id."') ) ";
    }
    $WHERE.=" AND sp.visiblity=1 AND wp_posts.post_type = 'product_variation' AND (wp_posts.post_status = 'publish' OR wp_posts.post_status = 'wc-deltoinstaller' OR wp_posts.post_status = 'wc-customprocess' OR wp_posts.post_status = 'future' OR wp_posts.post_status = 'draft' OR wp_posts.post_status = 'pending' OR wp_posts.post_status = 'private') GROUP BY wp_posts.ID ORDER BY sp.status desc";
    	...
    	$SQL=$SQL.$WHERE;
			...
			{	$myraSales=array();
				 $SQL="SELECT * FROM th_supplier_products_list WHERE product_id='".$value->product_id."' AND supplier_id=4";
				$myraSales=$wpdb->get_row($SQL);
				...
				$suppliers_products[$key]['supplier1']= ($myraSales->product_id) ? 'Yes' : 'No';
				...
				if($tyre_type == 'tubetyre')
	        	{
					...
					$suppliers_products[$key]['tube_price']= wc_price(get_post_meta($value->product_id,'tube_price',true));	
					}else{					
					$suppliers_products[$key]['tube_price']=wc_price(get_post_meta($value->product_id,'tyre_price',true));	
					}			
				}
				else{
				...
				if($value->new_tyre_price){
					$suppliers_products[$key]['tyre_price']=wc_price(get_post_meta($value->product_id,'tyre_price',true));		
				}else{					
				$suppliers_products[$key]['tyre_price']=wc_price(get_post_meta($value->product_id,'tyre_price',true));	
				}

				if($value->new_mrp){
					$suppliers_products[$key]['mrp_price']= wc_price(get_post_meta($value->product_id,'_regular_price',true));	
				}else{					
				$suppliers_products[$key]['mrp_price']= wc_price(get_post_meta($value->product_id,'_regular_price',true));
				}

				if($value->new_total_price){
					$suppliers_products[$key]['total_price']= wc_price(get_post_meta($value->product_id,'_sale_price',true));
				}else{					
				$suppliers_products[$key]['total_price']=wc_price(get_post_meta($value->product_id,'_sale_price',true));
				}

				$suppliers_products[$key]['percentage']= $value->flat_percentage;
				$suppliers_products[$key]['margin_price']= wc_price($value->margin_price);
	...
	function get_sortable_columns() {
	  $sortable_columns = array(
	    ...
	    'supplier1'  => array('supplier1',false),
	...
	function get_columns(){
	        $columns = array(
	            ...
	            'supplier1' => __( 'Supplier1', 'mylisttable' ),
...
function products_list(){
  ...
  $ProductListTable = new Products_List_Table();
  $supplier_id=$_REQUEST['supplier_id'];
  $res=get_supplier_data($supplier_id);
  ?>  
  <div class="wrap"><h2>Products List 
...
<!--  <div style="margin-top:10px;"><b>Supplier : <?=$res[0]->business_name;?></b> </div> -->
  ...
  <form method="post">
    ...
    <div class="prd-search" style="margin-top: 20px;">
            ...
            <select class="select-supplier" name="supplier_id">
                <option value="">Select Supplier</option>
                <?php 
                global $wpdb;
              	$SQL="SELECT * FROM `th_supplier_data`";
              	$restuls=$wpdb->get_results($SQL);
                foreach ($restuls as $res)
                {?>
                    <option value="<?=$res->supplier_data_id;?>" <?php if($res->supplier_data_id==$_POST['supplier_id']){ echo 'selected';}?>><?=$res->business_name;?></option>
    ...
    <?php $ProductListTable->prepare_items();?>
    <?php
