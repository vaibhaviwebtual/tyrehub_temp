...
abstract class Order_Document {
	...
	*/
	...
	public function get_pdf1() {
		...
		$pdf_maker = wcpdf_get_pdf_maker( $this->get_html1(), $pdf_settings );
		$pdf = $pdf_maker->output();
	...
	public function get_pdf2() {
		...
		$pdf_maker = wcpdf_get_pdf_maker( $this->get_html2(), $pdf_settings );
		$pdf = $pdf_maker->output();
	...
	public function get_pdf3() {
		...
		$pdf_maker = wcpdf_get_pdf_maker( $this->get_html3(), $pdf_settings );
		$pdf = $pdf_maker->output();
	...
	public function get_html1( $args = array() ) {
		do_action( 'wpo_wcpdf_before_html', $this->get_type(), $this );
		$default_args = array (
			'wrap_html_content'	=> true,
		);
		...
		$html = $this->render_template( $this->locate_template_file( "invoice1.php" ), array(
				'order' => $this->order,
				'order_id' => $this->order_id,
			)
		);
	...
	public function get_html2( $args = array() ) {
		do_action( 'wpo_wcpdf_before_html', $this->get_type(), $this );
		$default_args = array (
			'wrap_html_content'	=> true,
		);
		...
		$html = $this->render_template( $this->locate_template_file( "invoice2.php" ), array(
				'order' => $this->order,
				'order_id' => $this->order_id,
			)
		);
	...
	public function get_html3( $args = array() ) {
		do_action( 'wpo_wcpdf_before_html', $this->get_type(), $this );
		$default_args = array (
			'wrap_html_content'	=> true,
		);
		...
		$html = $this->render_template( $this->locate_template_file( "paid-invoice.php" ), array(
				'order' => $this->order,
				'order_id' => $this->order_id,
			)
		);
	...
	public function output_pdf( $output_mode = 'download', $document_type) {
		$pdf = $this->get_pdf();
			$upload_dir = wp_upload_dir();			
			$upload_base = trailingslashit( $upload_dir['baseurl'] );
			
			if($document_type=='credit-notes'){
				$filename='credit-note-'.$this->order_id.'.pdf';
			}else{
				$document_type='invoice';
				$filename=$this->get_filename();
			}
			$document = wcpdf_get_document( $document_type, (array) $this->order_id,true);			
				// get pdf data & store
			$tmp_base = $upload_base . 'wpo_wcpdf/attachments/'.$filename;		
			$pdf_url =$tmp_base;
			
		wcpdf_pdf_headers($filename, $output_mode, $pdf_url);
		echo $pdf;
		die();
	}
public function output_offline_pdf( $output_mode = 'download', $filename ) {
			$pdf = $this->get_pdf();
			$upload_dir = wp_upload_dir();			
			$upload_base = trailingslashit( $upload_dir['baseurl'] );
			$document_type='invoice';
...
public function output_paid_invoice_pdf( $output_mode = 'download', $filename ) {
			$pdf = $this->get_pdf();
			$upload_dir = wp_upload_dir();			
			$upload_base = trailingslashit( $upload_dir['baseurl'] );
			$document_type='invoice';
	...
	public function output_pdf1( $output_mode = 'download' ) {
		$pdf = $this->get_pdf1();
			$upload_dir = wp_upload_dir();			
			$upload_base = trailingslashit( $upload_dir['baseurl'] );
			$document_type='invoice';
			$document = wcpdf_get_document( $document_type, (array) $this->order_id, true );			
				// get pdf data & store
			$tmp_base = $upload_base . 'wpo_wcpdf/attachments/'.$this->get_filename1();		
			
		wcpdf_pdf_headers( $this->get_filename1(), $output_mode, $pdf_url);
		echo $pdf;
		die();
	}

	public function output_pdf2( $output_mode = 'download' ) {
		$pdf = $this->get_pdf2();
		...
		$upload_dir = wp_upload_dir();			
			...
			$document = wcpdf_get_document( $document_type, (array) $this->order_id, true );			
				// get pdf data & store
			$tmp_base = $upload_base . 'wpo_wcpdf/attachments/'.$this->get_filename();		
			$pdf_url =$tmp_base;

		wcpdf_pdf_headers( $this->get_filename(), $output_mode, $pdf_url);
		echo $pdf;
		die();
	}

	public function output_pdf3( $output_mode = 'download' ) {
		$pdf = $this->get_pdf3();
			$upload_dir = wp_upload_dir();			
			$upload_base = trailingslashit( $upload_dir['baseurl'] );
			$document_type='invoice-voucher';
	...
	public function get_filename1( $context = 'download', $args = array() ) {
		$order_count = isset($args['order_ids']) ? count($args['order_ids']) : 1;
		...
		$filename = $name . '-voucher-' . $suffix . '.pdf';
